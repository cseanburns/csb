...
create_html_page() {
    ...
    # html, head
    {
        ...
        if [[ $index == no ]]; then
            ...
            if [[ -z $timestamp ]]; then
                echo -n "<time class=\"subtitle\">$(LC_ALL=$date_locale date +"$date_format")"
            else
                echo -n "<time class=\"subtitle\">$(LC_ALL=$date_locale date +"$date_format" --date="$timestamp")"
            fi
...
all_posts() {
...

    {
        ...
        echo "" 1>&3
        echo "</ul>"
        echo "<p><a href=\"./$index_file\">$template_archive_index_page</a></p>"
    } 3>&1 >"$contentfile"
...
rebuild_index() {
    ...
    # Create the content file
    {
        ...
        echo "<p><a href=\"$archive_index\">$template_archive</a> &mdash; <a href=\"$tags_index\">$template_tags_title</a> &mdash; <a href=\"$feed\">$template_subscribe</a></p>"
    } 3>&1 >"$contentfile"
...
# $1 the html file
get_post_title() {
    awk '/<h3><a href=".+">/, /<\/a><\/h3>/{if (!/<h3><a href=".+">/ && !/<\/a><\/h3>/) print}' "$1"
}
...
create_includes() {
    ...
    if [[ -f $footer_file ]]; then cp "$footer_file" .footer.html
        ...
        echo "<footer>$global_license <a href=\"$global_author_url\">$global_author</a> &mdash; <a href=\"mailto:$protected_mail\">$protected_mail</a><br/>"
